...
import { Calendar, Clock, MapPin, User, Mail, Phone, ChevronLeft, ChevronRight, Check, Loader2 } from "lucide-react";
...
export default function BookingPage() {
  ...
  // Fetch busy slots for next 3 months
  useEffect(() => {
    if (!tenantSlug) return;
    const from = new Date();
    const to = new Date(Date.now() + 90 * 24 * 3600 * 1000);
    setLoadingBusy(true);
    fetch(`/api/bookings/public/${tenantSlug}/busy?from=${from.toISOString()}&to=${to.toISOString()}`)
      .then((r) => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/bookings/public/${tenantSlug}/request`, {
      method: "POST",
      ...
      <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", fontFamily: "Poppins, sans-serif" }}>
        <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "48px 40px", maxWidth: 480, width: "100%", textAlign: "center" }}>
          ...
          <p style={{ color: "#a0a0a0", fontSize: 14, lineHeight: 1.6 }}>
            La tua richiesta è stata ricevuta. Riceverai una conferma via email non appena verrà approvata.
          </p>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "Poppins, sans-serif", padding: "32px 16px" }}>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ width: 40, height: 40, background: "#F5A623", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16, fontWeight: 900, color: "#000", fontSize: 18 }}>F</div>
          <h1 style={{ color: "#f5f5f5", fontSize: 26, fontWeight: 700, margin: "0 0 6px" }}>Richiedi una prenotazione</h1>
          <p style={{ color: "#a0a0a0", fontSize: 14, margin: 0 }}>Seleziona una data e compila il modulo. Riceverai una risposta via email.</p>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24 }}>
                <h3 style={{ color: "#f5f5f5", fontSize: 14, fontWeight: 600, margin: "0 0 16px", display: "flex", alignItems: "center", gap: 8 }}>
                  <Calendar size={15} color="#F5A623" /> Tipo di evento
                </h3>
                  ...
                  {EVENT_TYPES.map((et) => (
                    <button
                      ...
                      style={{
                        padding: "10px 14px",
                        borderRadius: 10,
                        border: eventType === et.value ? "1.5px solid #F5A623" : "1.5px solid rgba(255,255,255,0.08)",
                        background: eventType === et.value ? "rgba(245,166,35,0.12)" : "#1a1a1a",
                        color: eventType === et.value ? "#F5A623" : "#a0a0a0",
                        fontSize: 13,
                        fontWeight: 500,
                        cursor: "pointer",
                        textAlign: "left",
                  ...
                  {SERVICES.map((s) => (
                    <button
                      ...
                      style={{
                        padding: "8px 16px",
                        borderRadius: 8,
                        border: services.includes(s.value) ? "1.5px solid #F5A623" : "1.5px solid rgba(255,255,255,0.08)",
                        background: services.includes(s.value) ? "rgba(245,166,35,0.12)" : "#1a1a1a",
                        color: services.includes(s.value) ? "#F5A623" : "#a0a0a0",
                        fontSize: 13,
                        fontWeight: 500,
                        cursor: "pointer",
                        transition: "all 0.15s",
              ...
              <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24 }}>
                <h3 style={{ color: "#f5f5f5", fontSize: 14, fontWeight: 600, margin: "0 0 16px", display: "flex", alignItems: "center", gap: 8 }}>
                  <Clock size={15} color="#F5A623" /> Orario di inizio
                </h3>
                <input
                  type="time"
                  value={time}
                ...
                />
                {selectedEventType && (
                  <p style={{ color: "#666", fontSize: 12, marginTop: 8, margin: "8px 0 0" }}>
                    Durata stimata: {selectedEventType.duration}h
                  </p>
                )}
              </div>
              ...
              <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24 }}>
                <h3 style={{ color: "#f5f5f5", fontSize: 14, fontWeight: 600, margin: "0 0 16px", display: "flex", alignItems: "center", gap: 8 }}>
                  <User size={15} color="#F5A623" /> I tuoi dati
                </h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    <label style={labelStyle}>Nome e cognome *</label>
                    <input placeholder="Mario Rossi" value={clientName} onChange={(e) => setClientName(e.target.value)} required style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Email *</label>
                    <input type="email" placeholder="mario@esempio.it" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} required style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Telefono</label>
                    <input type="tel" placeholder="+39 333 1234567" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Luogo dell'evento</label>
                    <input placeholder="Milano, Piazza Duomo..." value={eventLocation} onChange={(e) => setEventLocation(e.target.value)} style={inputStyle} />
                  </div>
                  <div>
            ...
            <div>
              <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24, position: "sticky", top: 24 }}>
                <h3 style={{ color: "#f5f5f5", fontSize: 14, fontWeight: 600, margin: "0 0 20px", display: "flex", alignItems: "center", gap: 8 }}>
                  <Calendar size={15} color="#F5A623" /> Scegli una data
                </h3>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: "8px 4px",
                            borderRadius: 8,
                            border: isSelected ? "1.5px solid #F5A623" : "1.5px solid transparent",
                            background: isSelected ? "rgba(245,166,35,0.2)" : isBusy ? "rgba(239,68,68,0.06)" : "transparent",
                            color: disabled ? (isBusy ? "rgba(239,68,68,0.4)" : "#333") : isSelected ? "#F5A623" : "#f5f5f5",
                            fontSize: 13,
                ...
                <div style={{ display: "flex", gap: 16, marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ width: 10, height: 10, background: "rgba(245,166,35,0.2)", border: "1.5px solid #F5A623", borderRadius: 3 }} />
                ...
                {selectedDate && (
                  <div style={{ background: "rgba(245,166,35,0.08)", border: "1px solid rgba(245,166,35,0.2)", borderRadius: 10, padding: "12px 16px", marginTop: 16 }}>
                    <p style={{ color: "#F5A623", fontSize: 13, fontWeight: 600, margin: 0 }}>
